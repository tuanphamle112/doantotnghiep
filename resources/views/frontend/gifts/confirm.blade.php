    ...
    <div class="row">
        <section class="content full-width">
            <div class="submit_recipe container">
                <form action="{{ route('gift.post-confirm', $gift->id) }}" class="wrap-create-form" method="post">
                    {{ csrf_field() }}
                    <div class="wrap-main-image">
                        <div class="wrap-note">
                            <label for="note" style="margin-bottom:20px; margin:auto;">
                                <b style="color:red">Note:</b> You are try to change <span
                                    style="color: red">{{ $gift->star_point }}</span>
                                <i class="fa fa-star" style="color:#e68609"></i> to take this. Make sure all these
                                information below are correct!
                            </label>
                        </div>
                        ...
                        <div class="wrap-item-confirm">
                            ...
                            <div class="confirm-item confirm-user">
                                <div class="user-info-gift">
                                    @if ($user->avatar !== null)
                                    <img src="{{ asset('uploads/avatars/' . $user->avatar) }}" width="270" height="270"
                                        alt="{{ __('Profile Photo') }}">
                                    <a href="{{ route('profile.index', $user->id) }}">{{ $user->name }}</a>
                                    @else
                                    <img src="{{ config('manual.default_media.avatar.man') }}" width="270" height="270"
                                        alt="{{ __('Profile Photo') }}">
                                    <a href="{{ route('profile.index', $user->id) }}">{{ $user->name }}</a>
                                    @endif
                                </div>
